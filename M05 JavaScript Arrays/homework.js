...
function arrayContiene(array, elemento) {
   ...
   // Tu código:
   return array.some(array => array === elemento)
}
...
function agregarNumeros(arrayOfNums) {
   ...
   // Tu código:
   return arrayOfNums.reduce((array, sum) => array + sum, 0);
}
...
function promedioResultadosTest(resultadosTest) {
   ...
   while(i < resultadosTest.length) {
      accu = accu + resultadosTest[i]
      i++
   }
   return accu/resultadosTest.length;
}
...
function todosIguales(array) {
   ...
   j=1;
   let band = true;
   while(i < array.length-1 && band === true) {
      if(array[i] !== array[j]){
         band = false;
      } 
      i++
